...
namespace AService.Controllers
{
	...
	public class BookStoreController : ControllerBase
	{
		...
		{
			using (this._logger?.BeginScope("GetItems"))
			{
				this._logger?.LogDebug("All items requested");
		...
		{
			using (this._logger?.BeginScope("GetItem ({ID})", id))
			{
				this._logger?.LogDebug("Book with id {ID} requested", id);
		...
		{
			using (this._logger?.BeginScope("GetBookName {ID}", id))
			{
				this._logger?.LogDebug("Name of book with id {ID} requested", id);
		...
		{
			using (this._logger?.BeginScope("PutItem {ID}", id))
			{
				this._logger?.LogDebug("Book data update with id {ID} requested", id);
		...
		{
			using (this._logger?.BeginScope("PostItem"))
			{
				this._logger?.LogDebug("Request to insert new book data");
		...
		{
			using(this._logger?.BeginScope("DeleteItem {ID}", id))
			{
				this._logger?.LogDebug("Request to remove book with id {ID} from store", id);
